...
/**
 * The gui for the heads in the miscellaneous category
 *
 * @since 4.0.0
 */
public class MiscHeadsMenu extends Gui {
    ...
    public MiscHeadsMenu() {
		...
		addItem(diamondSteveHead, event -> {
            event.setCursor(event.getCurrentItem());
            event.setCancelled(true);
        });
		...
		addItem(lightBrownMushroom, event -> {
            event.setCursor(event.getCurrentItem());
            event.setCancelled(true);
        });
		...
		addItem(commandBlockMinecart, event -> {
            event.setCursor(event.getCurrentItem());
            event.setCancelled(true);
        });
		...
		addItem(unitedStatesOfAmerica, event -> {
            event.setCursor(event.getCurrentItem());
            event.setCancelled(true);
        });
