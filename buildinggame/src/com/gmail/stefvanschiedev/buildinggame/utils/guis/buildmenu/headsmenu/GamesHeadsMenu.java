...
/**
 * The gui for the heads in the games category
 *
 * @since 4.0.0
 */
public class GamesHeadsMenu extends Gui {
    ...
    public GamesHeadsMenu() {
		...
		addItem(portalCoreWheatley, event -> {
            event.setCursor(event.getCurrentItem());
            event.setCancelled(true);
        });
		...
		addItem(solvedRubiksCube, event -> {
            event.setCursor(event.getCurrentItem());
            event.setCancelled(true);
        });
		...
		addItem(scrambledRubiksCube, event -> {
            event.setCursor(event.getCurrentItem());
            event.setCancelled(true);
        });
