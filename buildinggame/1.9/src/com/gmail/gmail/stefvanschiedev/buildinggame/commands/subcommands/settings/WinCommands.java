...
import com.gmail.stefvanschiedev.buildinggame.commands.subcommands.settings.wincommands.Add;
import com.gmail.stefvanschiedev.buildinggame.commands.subcommands.settings.wincommands.Remove;
import com.gmail.stefvanschiedev.buildinggame.managers.messages.MessageManager;

public class WinCommands extends ConsoleCommand {
		...
		if (args.length == 0) {
			for (SubCommand sc : subCommands) {
				if (sender.hasPermission(sc.getPermission()))
					MessageManager.getInstance().sendWithoutPrefix(sender, ChatColor.GREEN + "/bg setting win-commands " + sc.getName() + " - " + sc.getInfo());
			}
	...
	public String getInfo() {
		return "Change the win commands";
	}

	@Override
	public String getPermission() {
		return "bg.setting.wincommands";
	}
}